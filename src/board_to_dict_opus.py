...
# Reads ANTHROPIC_API_KEY from the environment -- never hardcode a real key in
...
def generate_state_dictionary(*, anthropic_api_key: str) -> dict:
    client = Anthropic(api_key=anthropic_api_key)
    base64_image = encode_image(image_path, max_dimension=1500, crop_box=crop_box)
    response = client.messages.create(
        model="claude-opus-5",
        max_tokens=8000,  # covers adaptive thinking + the visible JSON output combined
        output_config={"effort": "high"},  # matches the prior reasoning_effort="high"
        messages=[
            {
                "role": "user",
                "content": [
    ...
    # response.content can include a thinking block (adaptive thinking is on by
    # default for Opus 5) alongside the visible text block -- only the text
    # block holds the JSON we want.
    raw_output = ""
    ...
    if stop_reason == "max_tokens":
        print("\nWARNING: response was truncated -- raise max_tokens.")
