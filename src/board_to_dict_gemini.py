...
# Reads GOOGLE_API_KEY from the environment -- never hardcode a real key in
...
def generate_state_dictionary(*, google_api_key: str) -> dict:
    client = genai.Client(api_key=google_api_key)
    base64_image = encode_image(image_path, max_dimension=1500, crop_box=crop_box)
    image_bytes = base64.b64decode(base64_image)  # Part.from_bytes wants raw bytes
    ...
    response = client.models.generate_content(
        ...
        config=types.GenerateContentConfig(
            max_output_tokens=8000,  # caps thinking + visible output combined
            thinking_config=types.ThinkingConfig(
                thinking_level=types.ThinkingLevel.HIGH  # matches the prior reasoning_effort="high"
            ),
        ),
    )
    ...
    if finish_reason == types.FinishReason.MAX_TOKENS:
        print("\nWARNING: response was truncated -- raise max_output_tokens.")
