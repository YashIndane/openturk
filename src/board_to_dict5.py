...
TOP-DOWN RIM TEST (vs. Bishop): You must clearly see a repeating, regular ring of many small scalloped points around the entire rim -- like a gear or star -- before calling this a Queen. If the rim is smooth and continuous with at most one straight groove cutting into it, that is a Bishop's cleft, not a Queen's fluting, regardless of how tall the piece looks or how the lighting reads on a dark piece. When in doubt on a black piece, look for the repeating regularity of the pattern: many small evenly-spaced points = Queen; one irregular straight cut on an otherwise smooth surface = Bishop.
...
- If a color shows 2+ Queens: you have a Queen/Bishop misread somewhere. Revisit those Queen calls and check the rim test again -- keep the one(s) with genuine repeating scalloped fluting as Queen, and correct the rest to Bishop if their rim was actually smooth with a single cleft.
