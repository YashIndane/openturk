...
def main():
    parser = argparse.ArgumentParser(description="Chess picker move controller")
    parser.add_argument("coor", nargs="?", help="Move in the form 'e8e7' (source+destination)")
    parser.add_argument("--capture", action="store_true", help="Treat this as a capture move")
    parser.add_argument("--speed", type=float, default=DEFAULT_MAX_SPEED_MMS, help="Cruise speed in mm/s")
    parser.add_argument("--accel", type=float, default=DEFAULT_ACCEL_MMS2, help="Acceleration in mm/s^2")
    parser.add_argument("--calibrate", action="store_true", help="Run the axis-direction calibration test")
    ...
    try:
        if args.calibrate:
            ...
            move_picker(args.coor, capture=args.capture,
                        max_speed_mms=args.speed, accel_mms2=args.accel)
            print("Move complete.")
        else:
            parser.print_help()
