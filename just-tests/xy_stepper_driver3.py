...
def _play_pulses_streaming(pi, pulses):
    """
    Plays a long pulse list as a sequence of chunked waves, using
    double-buffering: the *next* chunk is built and ready to fire
    while the *current* chunk is still playing. Switching chunks then
    only costs a quick status check instead of the time it takes to
    build a whole new wave, which is what was causing the audible
    stutter/vibration on longer moves with the previous "finish, then
    build next" approach. Only two chunks' worth of DMA control blocks
    are ever alive at once, so this still works for any move length.
    """
    if not pulses:
        return
    ...
    try:
        ...
        # in case of an unexpected error mid-stream, don't leak wave ids
        if next_wid is not None:
            pi.wave_delete(next_wid)
...
def move_axes(pi, moves, max_speed_mms=DEFAULT_MAX_SPEED_MMS, accel_mms2=DEFAULT_ACCEL_MMS2):
    ...
    # Stream the pulses using double-buffered chunks (see _play_pulses_streaming).
    try:
        _play_pulses_streaming(pi, pulses)
    finally:
