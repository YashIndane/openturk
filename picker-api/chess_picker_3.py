...
# Captured pieces are no longer all dropped at the single 'X' point above.
# Instead they're placed one at a time along the margin row from h10 down
# to a10; once a10 is used, the sequence wraps back around to h10 for the
# next capture.
DROP_FILES = "hgfedcba"
DROP_RANK_IDX = ORIGIN_RANK_IDX   # same margin row as the park square

_drop_index = 0   # advances by one every capture; wraps via modulo


def _next_drop_square():
    """Returns the next square in the h10->a10 capture drop-off sequence
    (wrapping back to h10 after a10), and advances the sequence so the
    following capture uses the next square along."""
    global _drop_index
    file_char = DROP_FILES[_drop_index % len(DROP_FILES)]
    _drop_index += 1
    return f"{file_char}{DROP_RANK_IDX}"
...
def move_picker(coor, capture=False,
    ...
    cleared first):
        PARK_SQUARE->dst, pick up, dst->drop, place down (discard captured piece),
        drop->src, pick up, src->dst, place down, dst->PARK_SQUARE
    where "drop" is the next square in the sequential h10->a10 capture
    drop-off sequence (see _next_drop_square()), wrapping back to h10
    once a10 has been used.
    """
